// providers/update_equipment_provider.dart
...
// State for Update Equipment
class UpdateEquipmentState {
...
// Update Equipment Notifier
class UpdateEquipmentNotifier extends StateNotifier<UpdateEquipmentState> {
  ...
  UpdateEquipmentNotifier(
    this._controller,
    this._categoryController,
    this._levelController,
    this._ref,
  ) : super(UpdateEquipmentState()) {
    _loadInitialData();
  }
  ...
  Future<String> updateEquipment({
    ...
    if (state.equipment == null || state.equipment!.equipmentId == null) {
      state = state.copyWith(message: 'Equipment not found', isSuccess: false);
      return 'Status 3000';
    }
    ...
    try {
      ...
      final result = await _controller.updateEquipment(
      ...
      if (result == 'Status 1000') {
        ...
        switch (result) {
          ...
          case 'Status 6000':
            errorMessage =
                'You are not authorized to update equipment records.';
            break;
          case 'Status 2000':
            errorMessage = 'Server error. Please try again later.';
            break;
          default:
            errorMessage = 'Unexpected error: $result';
            break;
        }
        state = state.copyWith(
          isLoading: false,
...
// Provider for UpdateEquipmentNotifier
final updateEquipmentProvider =
    StateNotifierProvider<UpdateEquipmentNotifier, UpdateEquipmentState>((ref) {
      ...
      return UpdateEquipmentNotifier(
        equipmentController,
        categoryController,
        levelController,
        ref,

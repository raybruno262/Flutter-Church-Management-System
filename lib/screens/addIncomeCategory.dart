...
class AddIncomeCategoryScreen extends StatefulWidget {
  ...
  const AddIncomeCategoryScreen({super.key, required this.loggedInUser});
...
class _AddIncomeCategoryScreenState extends State<AddIncomeCategoryScreen> {
  ...
  Future<void> _addIncomeCategory() async {
    ...
    try {
      final newIncomeCategory = IncomeCategory(name: incomeCategoryName);

      final result = await incomeCategoryController.createIncomeCategory(
        newIncomeCategory,
      );

      if (result == 'Status 1000') {
        setState(() {
          _message = 'Income Category created successfully!';
          _isSuccess = true;
        });
      ...
      setState(() {
        _message = 'Error creating Income Category';
        _isSuccess = false;
      });
    }
  }
          ...
          children: [
            ...
            Expanded(
              child: Container(
                ...
                child: _buildAddIncomeCategoryScreen(),
              ),
            ),
          ],
        ),
  ...
  Widget _buildAddIncomeCategoryScreen() {
    return Column(
